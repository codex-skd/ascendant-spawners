...
public class SpawnerStats {
    public static final ResourceKey<Registry<SpawnerStat<?>>> REGISTRY_KEY = ResourceKey.createRegistryKey(AscendantSpawners.loc("spawner_stat"));
    public static final Registry<SpawnerStat<?>> REGISTRY = new RegistryBuilder<>(REGISTRY_KEY).sync(true).create();
    ...
    public static final SpawnerStat<Float> INITIAL_HEALTH = register("initial_health", new PercentageStat(1.0F));
    ...
    public static void generateTooltip(SpawnerTile tile, Consumer<Component> list) {
    ...
    private static <T extends SpawnerStat<?>> T register(String id, T t) {
        Registry.register(REGISTRY, AscendantSpawners.loc(id), t);
        return t;
    }
}
