...
public class SpawnerCategory implements IRecipeCategory<SpawnerModifier> {
    ...
    public static final ResourceLocation TEXTURES = AscendantSpawners.loc("textures/gui/spawner_jei.png");
    public static final ResourceLocation UID = AscendantSpawners.loc("spawner_modifiers");
    public static final RecipeType<SpawnerModifier> TYPE = RecipeType.create(AscendantSpawners.MODID, "spawner_modifiers", SpawnerModifier.class);
    ...
    public void draw(SpawnerModifier recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics gfx, double mouseX, double mouseY) {
        ...
        if (mouseX >= -1 && mouseX < 9 && mouseY >= 13 && mouseY < 13 + 12) {
            ...
            gfx.renderComponentTooltip(font, Arrays.asList(AscendantSpawners.lang("misc", "mainhand")), (int) mouseX, (int) mouseY);
        }
            ...
            text.add(AscendantSpawners.lang("misc", "offhand"));
            if (!recipe.consumesOffhand()) {
                text.add(AscendantSpawners.lang("misc", "not_consumed").withStyle(ChatFormatting.GRAY));
            }
            gfx.renderComponentTooltip(font, text, (int) mouseX, (int) mouseY);
        }
        else if (mouseX >= 33 && mouseX < 33 + 16 && mouseY >= 30 && mouseY < 30 + 16) {
            gfx.renderComponentTooltip(font, Arrays.asList(AscendantSpawners.lang("misc", "rclick_spawner")), (int) mouseX, (int) mouseY);
        }
        ...
        for (StatModifier<?> s : recipe.getStatModifiers()) {
            ...
            Component msg = switch (s.mode()) {
                ...
                case SET -> {
                    if (s.value() instanceof Number) {
                        yield AscendantSpawners.lang("misc", "value_concat", s.stat().name(), value);
                    }
                    else {
                        if ("true".equals(value)) {
                            yield AscendantSpawners.lang("misc", "on", s.stat().name());
                        }
                        else {
                            yield AscendantSpawners.lang("misc", "off", s.stat().name());
                        }
                    }
                }
            };
            ...
            if (hover) {
                ...
                if (s.value() instanceof Number && s.mode() == StatModifier.Mode.ADD) {
                    ...
                    if (s.min().isPresent()) list.add(AscendantSpawners.lang("misc", "min_value", n.stat().formatValue(n.min().get())).withStyle(ChatFormatting.GRAY));
                    if (s.max().isPresent()) list.add(AscendantSpawners.lang("misc", "max_value", n.stat().formatValue(n.max().get())).withStyle(ChatFormatting.GRAY));
                }
