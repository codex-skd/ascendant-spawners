...
public class AscSpEvents {
    ...
    public void handleCapturing(LivingDropsEvent e) {
        ...
        if (killer instanceof LivingEntity living) {
            Pair<Unit, Integer> level = EnchantmentHelper.getHighestLevel(living.getWeaponItem(), AscSpObjects.CAPTURING);
            if (level == null || killed.getType().is(AscSpObjects.BLACKLISTED_FROM_SPAWNERS)) {
                return;
            }

            if (killed.level().random.nextFloat() < level.getSecond() * AscSpConfig.capturingDropChance) {
    ...
    public void handleUseItem(RightClickBlock e) {
        if (e.getLevel().getBlockEntity(e.getPos()) instanceof AscendantSpawnerTile) {
            ItemStack s = e.getItemStack();
            if (s.getItem() instanceof SpawnEggItem egg) {
                EntityType<?> type = egg.getType(s);
                if (type.is(AscSpObjects.BLACKLISTED_FROM_SPAWNERS)) {
                    e.setCanceled(true);
                }
            }
        }
    ...
    public void onDespawn(MobDespawnEvent e) throws Throwable {
        ...
        if (MobSpawnType.isSpawner(mob.getSpawnType()) && AscSpConfig.entityDespawnDelay >= mob.tickCount) {
