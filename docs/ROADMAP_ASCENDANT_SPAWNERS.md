...
| **4** | **Contenido data-driven y arte propio**: claves de idioma (`en_us.json`), los 14 JSON de advancement (deliberadamente excluidos de la Fase 1), tags estáticas (`blacklisted_from_spawners`, `non_treasure`), ejecutar `runData` para generar loot table/recetas/encantamiento desde los providers ya portados, y las 2 texturas propias (fondo de advancement + fondo JEI) | — (datos + assets, no clases Java) | Fase 1, 2, 3 |
